...
plot.df<-mapply( function(x, y)
   x %>%
        ...
        mutate(days_since_dialysis = as.numeric(as.Date(paste0("1jan",as.numeric(y)+1), "%d%b%Y") - as.Date(dialysis_date))) %>%  #time 
        mutate(days_since_tx = as.numeric(as.Date(paste0("1jan",as.numeric(y)+1), "%d%b%Y") - as.Date(kidney_tx_date))) %>%
        mutate(days_since_RRT = as.numeric(as.Date(paste0("1jan",as.numeric(y)+1), "%d%b%Y") - as.Date(RRT_date))) %>%
...
plot.df<-mapply(function(x,y)
   x %>%
        ...
        mutate(days_since_last_RRT = as.Date(paste0("1jan",as.numeric(y)+1), "%d%b%Y") - as.Date(latest_renal_date)),
   dflist, years, SIMPLIFY = FALSE)
