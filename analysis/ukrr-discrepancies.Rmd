...
ckd_codelist<-read.csv(here::here("codelists", "opensafely-chronic-kidney-disease-snomed.csv")) 
ckd_primis_1_5_codelist<-read.csv(here::here("codelists", "user-Louis-ckd-stage.csv")) 
dialysis_codelist<-read.csv(here::here("codelists", "opensafely-dialysis.csv")) 
kidney_tx_codelist<-read.csv(here::here("codelists", "opensafely-kidney-transplant.csv")) 
RRT_codelist<-read.csv(here::here("codelists", "opensafely-renal-replacement-therapy.csv")) 
...
```
For some people the latest renal code relates to CKD but there is a previous RRT code.  The tables below describe the most recent codes for each OpenSAFELY primary care codelist.
```{r, echo=FALSE, results='asis'}
#Then the latest codes for CKD, dialysis, transplant and RRT
codes.table<-function(df,codevar,codelist){
  df %>%
    filter(rrt_source=="UKRR only" & latest_renal_status!="None") %>%
    count(!!as.name(codevar)) %>%
    left_join(codelist, by=setNames(names(codelist)[1], codevar)) %>%
    mutate(!!codevar:= replace(!!as.name(codevar), !!as.name(codevar)=="", "Missing")) %>%
    mutate(across(where(is.numeric)&!c(1), ~ roundup(.x,5))) %>% 
    mutate(across(where(is.numeric)&!c(1),~ifelse(.<=5,"<=5",.)))
}
...
for(i in 1:length(list.of.dts)) {
  ...
  #  colnames(list.of.dts[[i]][[j]])<-rep("",length(names)*2) # blank colnames
   k<- list.of.dts[[i]][[j]]%>%
          kbl(caption=paste0("Table 1.", i, "." , j, " Frequency of most recent ", all.code.vars[j], "s, ", years[i] ), booktabs=TRUE) %>% 
...
```
For people with a RRT code which predates the latest CKD code, we look at how long it has been since the last RRT code.
```{r, echo=FALSE, results='asis'}
...
plot.df<-mapply( function(x, y)
   x %>%
        filter(rrt_source=="UKRR only" & 
                 (dialysis==1 | kidney_tx==1 | RRT==1)) %>%
        mutate(days_since_dialysis = as.numeric(as.Date(paste0("1jan",as.numeric(y)+1), "%d%b%Y") - as.Date(dialysis_date, "%Y-%m-%d"))) %>%  #time 
        mutate(days_since_tx = as.numeric(as.Date(paste0("1jan",as.numeric(y)+1), "%d%b%Y") - as.Date(kidney_tx_date, "%Y-%m-%d"))) %>%
        mutate(days_since_RRT = as.numeric(as.Date(paste0("1jan",as.numeric(y)+1), "%d%b%Y") - as.Date(RRT_date, "%Y-%m-%d"))) %>%
        mutate(days_since_last_RRT = pmin(days_since_dialysis, days_since_tx, days_since_RRT)),
    dflist, years, SIMPLIFY = FALSE)  


timevars<-c("days_since_dialysis", "days_since_tx", "days_since_RRT", "days_since_last_RRT")
titles<-c(paste("Histogram of days since", 
              c("last dialysis codelist code", "last transplant codelist code", "last RRT codelist code"), 
              "and end of"),
          "Histogram of time between the most recent RRT code (any codelist) and end of")

mapply(function(z, d) 
  mapply(function(x, y) 
    ggplot(z, aes_string(x)) + geom_histogram() +
      labs(title=paste(y, d))  + theme(plot.title = element_text(size = 10, face = "bold")),
    timevars, titles, SIMPLIFY =FALSE  ), plot.df, years, SIMPLIFY =FALSE  )

#May want to show this by code later
...
Below we look at what the most recent primary care codes are for these patients, by codelist. The RRT groups is patients where the RRT code did not specify dialysis or transplant.
```{r, echo=FALSE, results='asis'}
...
codes.table.2<-function(df,status, codelist){
  df %>%
    ...
    mutate(latest_renal_code= replace(latest_renal_code, latest_renal_code=="", "Missing")) %>%
    mutate(across(where(is.numeric)&!c(1), ~ roundup(.x,5))) %>% 
    mutate(across(where(is.numeric)&!c(1),~ifelse(.<=5,"<=5",.)))
}
...
plot.df<-mapply(function(x,y)
   x %>%
        filter(rrt_source=="PC only" & latest_renal_status!="None") %>%
        mutate(days_since_last_RRT = as.Date(paste0("1jan",as.numeric(y)+1), "%d%b%Y") - as.Date(latest_renal_date, "%Y-%m-%d")),
   dflist, years, SIMPLIFY = FALSE)

mapply(function(z, d) 
  lapply( c("Dialysis", "Transplant", "RRT_unknown"), function(x) 
    ggplot(subset(z, latest_renal_status==x), aes(days_since_last_RRT)) + geom_histogram() +
