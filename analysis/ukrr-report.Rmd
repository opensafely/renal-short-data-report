...
Capture of RRT patients in each data source, by demographics. Numbers are rounded to the nearest 10, and percentages to zero decimal places.

```{r, echo=FALSE, results='asis'}
dtable<-function(x, y) {
    y1<-keepRRT(y)
    m1<-cbind(round(table(y1[,x],y1$rrt_source),-1),
              round(100*prop.table(table(y1[,x],y1$rrt_source), margin=1),0))
    m2 <- m1[, c(matrix(1:ncol(m1), nrow = 2, byrow = T))]  
