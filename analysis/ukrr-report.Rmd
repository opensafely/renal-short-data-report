...
#### Disclosure control
Cells less than or equal to 5 are shown as <=5 and corresponding percentages blanked. All other numbers are rounded to the nearest 5 and percentages calculated on the rounded number.
...
#table for numbers
ntable<-function(x){
  round(table(x$ukrr_prev, x$latest_renal_status %in% c("Dialysis", "Transplant", "RRT_unknown"))/5)*5 #round to nearest 5
}
...
#table for numbers
ntable<-function(x){
  round(table(keepRRT(x)$ukrr_dtx, keepRRT(x)$latest_dtx)/5)*5 #round to nearest 5
}
...
dtable<-function(x, y) {
    ...
    round.t<-round(noround.t/5)*5 #rounding to nearest 5
