...
This document describes the cohorts of patients prevalent to renal replacement therapy (RRT) at 1st January 2022 using 3 sources of data: the UK Renal Registry (UKRR), primary care data from TPP practices (PC) and secondary care inpatient and outpatient data (SC).  
...
#pick up 1st Jan cohort as latest renal status is on or before index date
 df<-read.csv(here::here("output", "ukrr", "input_ukrr_short_2022-01-01.csv.gz")) 
 

#add variable for dialysis/tx for UKRR
df<- df %>%
  mutate(ukrr_dtx = ifelse(ukrr_2021_mod %in% c("ICHD", "HD", "HHD", "PD") , "Dialysis", ifelse(ukrr_2021_mod == "Tx", "Transplant", "Not on RRT"))) %>%
...
df <- df %>%
  mutate(source_prev_rrt = case_when(ukrr_2021==1 & 
...
##  Prevalent at 1/1/2022, by data source and modality
### UKRR
```{r, echo=FALSE, results='asis', error=TRUE}
#create tables
list.of.dts<-lapply(df[df$ukrr_2021==1, demogs],
    function(x) disclosure(table(x, df[df$ukrr_2021==1,]$ukrr_dtx, useNA="ifany"),2))
#printing
...
## Prevalent at 1/1/2022, overall and by modality
### All RRT
```{r, echo=FALSE, results='asis', error=TRUE}
