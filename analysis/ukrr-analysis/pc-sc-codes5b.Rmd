...
t<- t(as.matrix(apply(CC_flags_inc, 2, function(x) disclosure(table(factor(x[is.na(inc_year_ukrr) & inc_year_sc==2020], levels=0:1))))))
  k<- t  %>%
          kbl(caption="Not in UKRR 2020", booktabs=TRUE) %>%
          kable_styling()
  print(k)
```
...
t<- t(as.matrix(apply(CC_flags_inc, 2, function(x) disclosure(table(factor(x[inc_year_ukrr==2020 & inc_year_sc==2020], levels=0:1))))))
  k<- t  %>%
          kbl(caption="In UKRR 2020", booktabs=TRUE) %>%
          kable_styling()
  print(k)
```