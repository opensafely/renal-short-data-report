...
list.of.dts<-lapply(df[df$ukrr_inc2020==1, demogs],
       function(x) addmargins(roundup(table(x, df[df$ukrr_inc2020==1,]$ukrr_dtx_inc, useNA="ifany"), 5),2))
#printing
for(i in 1:length(list.of.dts)) {
  k<- list.of.dts[[i]] %>%
...
list.of.dts<-lapply(df[df$incident_rrt_status  %in%  c("Dialysis", "Transplant", "RRT_unknown"), demogs],
       function(x) addmargins(roundup(table(x, df[df$incident_rrt_status  %in%  c("Dialysis", "Transplant", "RRT_unknown"),]$incident_rrt_status, useNA="ifany"),5),2))
#printing
for(i in 1:length(list.of.dts)) {
  k<- list.of.dts[[i]] %>%
