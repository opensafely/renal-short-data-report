...
at least because they have at least one RRT entry. Checking frequency of incident flags 

```{r, echo=FALSE, results='asis', error=TRUE}
t<-table(df_main$earliest_RRT, as.numeric(substr(df_main$earliest_RRT_date,1,4)), useNA="ifany")
  k<- t %>%
          kbl( booktabs=TRUE) %>%
          kable_styling()
  print(k)

t<-table(df_main$earliest_RRT, is.na(df_main$earliest_RRT_date) | df_main$earliest_RRT_date=="", useNA="ifany")
  k<- t %>%
          kbl( booktabs=TRUE) %>%
          kable_styling()
  print(k)
...
t<-table(df_main$earliest_dialysis, as.numeric(substr(df_main$earliest_dialysis_date,1,4)), useNA="ifany")
  k<- t %>%
          kbl( booktabs=TRUE) %>%
          kable_styling()
  print(k)

  t<-table(df_main$earliest_dialysis, is.na(df_main$earliest_dialysis_date) | df_main$earliest_dialysis_date=="", useNA="ifany")
  k<- t %>%
          kbl( booktabs=TRUE) %>%
          kable_styling()
  print(k)

  t<-table(df_main$earliest_kidney_tx, as.numeric(substr(df_main$earliest_kidney_tx_date,1,4)), useNA="ifany")
  k<- t %>%
          kbl( booktabs=TRUE) %>%
          kable_styling()
  print(k)

  t<-table(df_main$earliest_kidney_tx, is.na(df_main$earliest_kidney_tx_date) | df_main$earliest_kidney_tx_date=="", useNA="ifany")
  k<- t %>%
          kbl( booktabs=TRUE) %>%
          kable_styling()
  print(k)
