...
CC_vars<-names(df_check)[grepl('cc_flag', names(df_check))]
...
inc_year_ukrr<-as.numeric(substr(df_main$ukrr_inc2020_date))
inc_year_ukrr[is.na(inc_year_ukrr)]<-as.numeric(substr(df_main$ukrr_2019_startdate))
inc_year_ukrr[is.na(inc_year_ukrr)]<-as.numeric(substr(df_main$ukrr_2020_startdate))
inc_year_ukrr[is.na(inc_year_ukrr)]<-as.numeric(substr(df_main$ukrr_2021_startdate))
...
t<- t(as.matrix(apply(df_check[CC_vars], 2, function(x) table(factor(x, levels=0:1)))))
  k<- t  %>%
          kbl( booktabs=TRUE) %>%
          kable_styling()
  print(k)
