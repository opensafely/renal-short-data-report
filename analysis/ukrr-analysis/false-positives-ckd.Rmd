...
k<- table(df$egfr_numeric_value_history_operator)
k2<-k%>%
  kbl() %>%
  kable_styling()
print(k2)
```
### summarise numeric value by operator
```{r, echo=FALSE, results='asis', error=TRUE}
k<- do.call(rbind, tapply(df$egfr_numeric_value_history, df$egfr_numeric_value_history_operator, summary))
k2<-k%>%
  kbl() %>%
#  add_header_above(c("Min." ,"1st Qu." , "Median"  , "Mean" ,"3rd Qu." , " Max." )) %>%
  kable_styling()
print(k2)
```
...
Agreement is indicated by the data source, defined as UPS = in the **U**KRR, **P**rimary care and **S**econdary care data; UP = in **U**KRR and **P**rimary care only; U = in **U**KRR only etc.  Restricted to the latest PC eGFR is <15
