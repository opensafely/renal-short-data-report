...
SC_vars<-names(df)[grepl('proc_|diag_', names(df))]
#list of variables with critical care info
CC_vars<-names(df)[grepl('_cc_', names(df))]
...
```
  ...
  # Section 1 Codes for people on RRT according to primary care data but not the UKRR
Below we look at what the most recent primary care codes are for these patients, by modality
```{r, echo=FALSE, results='asis'}

codes.table.3<-function(df, grouplist, status, codelist){
  df %>%
...
```   
 ...
 # Section 2 Secondary care codes for people on RRT according to secondary care data but not the UKRR
This is done slightly differently as we have one variable per code

```{r, echo=FALSE, results='asis', error=TRUE}

list.of.dts<-lapply(df[SC_vars], function(x) table(x, df$ukrr_2019, useNA="ifany"))
...
t<- t(as.matrix(apply(CC_flags, 2, function(x) table(factor(x, levels=0:1),df$ukrr_2019, useNA="ifany"))))
  k<- t  %>%
          kbl( booktabs=TRUE) %>%
          kable_styling()
  print(k)
