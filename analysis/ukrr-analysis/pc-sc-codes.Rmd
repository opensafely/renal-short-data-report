                            ...
                            ifelse(next_before_allRRT=="", next_after_allRRT,
                                ifelse(next_after_allRRT=="", next_before_allRRT,
                                  ifelse(as.Date(ukrr_inc2020_date, "%Y-%m-%d") - as.Date(next_before_allRRT_date, "%Y-%m-%d")
                                       ...
                                       next_before_allRRT_date, next_after_allRRT_date))))))
...
df<-df %>%
    mutate(incident_rrt_code = ifelse(incident_rrt_status=="None", "None",
                                  ifelse(incident_rrt_status=="Dialysis", dialysis_code,
                                      ifelse(incident_rrt_status=="Transplant", kidney_tx_code,
                                             ifelse(incident_rrt_status=="RRT_unknown", RRT_code,"Uncategorised")))))
...
```    
And repeating for people who are in the UKRR, for reference
...
# Section 2 Secondary care codes for people prevalent to RRT according to secondary care data by whether or not they are prevalent in the UKRR

This is done slightly differently as we have one variable per code

```{r, echo=FALSE, results='asis', error=TRUE}

list.of.dts<-lapply(df[SC_vars_prev], function(x) disclosure(table(x, df$ukrr_2019, useNA="ifany")))

for(i in 1:length(list.of.dts)) {
  k<- list.of.dts[[i]] %>%
          kbl(caption=SC_vars_prev[i], booktabs=TRUE) %>%
          kable_styling()
  print(k)
}
```

Now for each code, looking at critical care days x whether they are in UKRR

```{r, echo=FALSE, results='asis', error=TRUE}

list.of.dts<- lapply(CC_flags_prev, function(x) disclosure(table(factor(x, levels=0:1),df$ukrr_2019)))
for(i in 1:length(list.of.dts)) {
  k<- list.of.dts[[i]] %>%
          kbl(caption=SC_vars_prev[i],  booktabs=TRUE) %>%
          kable_styling()
...
codes.table.3<-function(df, status, codelist){
  df %>%
    ...
    summarise(n=n()) %>%
    left_join(codelist, by=setNames(names(codelist)[1], "incident_rrt_code")) %>%
...
list.of.dts<-lapply(df[SC_vars_inc], function(x) disclosure(table(x[is.na(inc_year_ukrr)])))

for(i in 1:length(list.of.dts)) {
  k<- list.of.dts[[i]] %>%
          kbl(caption=SC_vars_inc[i], booktabs=TRUE) %>%
          kable_styling()
  print(k)
}
```
