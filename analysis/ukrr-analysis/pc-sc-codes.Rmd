...
df<-df%>%
  mutate(nearest_rrt_code = ifelse(ukrr_inc2020==0, "Not inc in UKRR,
                          ifelse(next_before_allRRT=="" and next_after_allRRT=="", "No events",
                            ifelse(next_before_allRRT=="", next_after_allRRT,
                                ifelse(next_after_allRRT=="", next_before_allRRT,
                                  ifelse(ukrr_inc2020_date - next_before_allRRT_date < next_after_allRRT_date - ukrr_inc2020_date, next_before_allRRT_date, next_after_allRRT_date))))))
...
for(i in 1:length(list.of.dts)) {
  k<- list.of.dts[[i]] %>%
          ...
          add_header_above(c("UKRR"=ncol(t)+1)) %>%
          kable_styling()
  print(k)
}
```
...
t<- t(as.matrix(apply(CC_flags_prev, 2, function(x) disclosure(table(factor(x, levels=0:1),df$ukrr_2019, useNA="ifany")))))
  k<- t  %>%
          kbl( booktabs=TRUE) %>%
          kable_styling()
  print(k)
